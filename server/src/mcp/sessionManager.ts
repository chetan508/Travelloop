...
export interface McpSession {
  ...
  userId: number;
  /** null = static traveloop_ token or JWT (full access); string[] = OAuth 2.1 scopes */
  scopes: string[] | null;
  /** OAuth 2.1 client_id that owns this session; null for static-token / JWT sessions */
  clientId: string | null;
  /** true when authenticated via static traveloop_ token — triggers deprecation prompt */
