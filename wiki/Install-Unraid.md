...
Install TRAVELOOP on Unraid via Community Applications or a direct template import.
...
2. Search for **TRAVELOOP**.
3. Click **Install** on the TRAVELOOP result.

If the app does not appear, you can install directly from the template URL. In **Docker → Add Container**, paste the template URL:

```
https://raw.githubusercontent.com/mauriceboe/TRAVELOOP/main/unraid-template.xml
```
...
| Data | `/app/data` | `/mnt/user/appdata/traveloop/data` |
| Uploads | `/app/uploads` | `/mnt/user/appdata/traveloop/uploads` |
...
| `ALLOWED_ORIGINS` | *(empty)* | Comma-separated origins for CORS and email notification links, e.g. `https://traveloop.example.com` |
...
| `ADMIN_PASSWORD` | *(empty)* | Password for the first admin account (first-boot only). Must be set together with `ADMIN_EMAIL`. If either is omitted, TRAVELOOP creates the account with email `admin@traveloop.local` and a random password printed to the container log. |
...
Copy the output into the `ENCRYPTION_KEY` field before starting the container for the first time. If you skip this, TRAVELOOP auto-generates a key and saves it to `data/.encryption_key` — your data is still protected, but you should record that file in your backups.
...
On first boot, TRAVELOOP automatically creates an admin account. The credentials are printed to the container log — check **Docker → traveloop → Log** in the Unraid UI. If you set both `ADMIN_EMAIL` and `ADMIN_PASSWORD`, those values are used; otherwise the email is `admin@traveloop.local` and a random password is generated.
