...
TRAVELOOP exposes **tools** (read and write actions) and **resources** (read-only `traveloop://` URIs). Tools are registered per-session based on OAuth scopes and enabled addons.
...
Resources provide read-only access via `traveloop://` URIs. Read them to understand current state before making changes.

### Core resources

| URI | Scope required | Description |
|---|---|---|
| `traveloop://trips` | `trips:*` | All trips you own or are a member of |
| `traveloop://trips/{tripId}` | `trips:*` | Single trip with metadata and member count |
| `traveloop://trips/{tripId}/days` | `trips:*` | Days of a trip with their assigned places |
| `traveloop://trips/{tripId}/places` | `places:read` | All places in a trip. Supports `?assignment=all\|unassigned\|assigned` |
| `traveloop://trips/{tripId}/reservations` | `reservations:read` | Flights, hotels, restaurants, and other reservations |
| `traveloop://trips/{tripId}/days/{dayId}/notes` | `trips:*` | Notes for a specific day |
| `traveloop://trips/{tripId}/accommodations` | `trips:*` | Hotels and rentals with check-in/out details |
| `traveloop://trips/{tripId}/members` | `trips:*` | Owner and collaborators |
| `traveloop://categories` | (any) | Available place categories (id, name, icon, color) |
| `traveloop://notifications/in-app` | `notifications:read` | Your in-app notifications (most recent 50, unread first) |
