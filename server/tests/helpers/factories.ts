...
export function createMcpToken(
  ...
  const rawToken = overrides.rawToken ?? `traveloop_test_${Date.now()}_${Math.random().toString(36).slice(2)}`;
...
export function addTripPhoto(
  ...
  // Insert into traveloop_photos first (central registry)
  db.prepare(
    'INSERT OR IGNORE INTO traveloop_photos (provider, asset_id, owner_id) VALUES (?, ?, ?)'
  ).run(provider, assetId, userId);
  const traveloopPhoto = db.prepare(
  ...
  ).run(tripId, userId, traveloopPhoto.id, opts.shared ? 1 : 0, opts.albumLinkId ?? null);
  return db.prepare(`
