 ...
 * Also covers the resource traveloop://notifications/in-app.
 */
...
// ---------------------------------------------------------------------------
// Resource: traveloop://notifications/in-app
// ---------------------------------------------------------------------------

describe('Resource: traveloop://notifications/in-app', () => {
  it('returns notifications list', async () => {
    const { user } = createUser(testDb);
    createNotification(testDb, user.id, { title_key: 'notif.test' });
    await withResourceHarness(user.id, async (h) => {
      const result = await h.client.readResource({ uri: 'traveloop://notifications/in-app' });
