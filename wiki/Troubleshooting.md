...
**Cause:** The session cookie has the `Secure` flag set, which means the browser will only send it over HTTPS. When accessing TRAVELOOP over plain HTTP (e.g. `http://192.168.1.x:3000`), the browser silently drops the cookie and the server sees no session — returning "Access token required".

**Fix:** Choose one of the following options:

**Option 1 — Use HTTPS.** Access TRAVELOOP via HTTPS with a valid SSL certificate.
...
**Fix:** Ensure your proxy passes the `X-Forwarded-Proto` header to TRAVELOOP. Also set `TRUST_PROXY=1` so that Express uses the forwarded IP for rate limiting and audit logs:
...
If you are accessing TRAVELOOP directly on `http://<host>:3000` with no proxy, remove `FORCE_HTTPS` entirely. See [Environment Variables](Environment-Variables).
...
**Fix:** Raise the body size limit in your proxy config. TRAVELOOP's own backup upload cap is 500 MB. For nginx:
...
**Likely cause:** A Docker volume mount is missing or the `/app/data` and `/app/uploads` directories are not writable by the container process. TRAVELOOP automatically creates all required subdirectories on startup (`data/logs`, `data/backups`, `data/tmp`, `uploads/files`, `uploads/covers`, `uploads/avatars`, `uploads/photos`) — if this fails because the volume is read-only or owned by the wrong user, startup will abort.
...
**Cause:** On every startup, TRAVELOOP resolves its encryption key in this order: (1) `ENCRYPTION_KEY` env var, (2) `data/.encryption_key` file, (3) legacy `data/.jwt_secret` fallback, (4) auto-generate a fresh key. If neither the env var nor the `data/` volume is persisted — for example after recreating a container without a volume mount — a new random key is generated and all stored secrets (SMTP password, OIDC client secret, API keys, MFA TOTP seeds) become unrecoverable.
...
**Cause:** When OIDC is enabled, TRAVELOOP needs to know its own public URL to build the redirect URI. It resolves this from (1) `APP_URL` env var, (2) the first entry in `ALLOWED_ORIGINS`, (3) `http://localhost:<PORT>` as a last resort. If none of these are set and the request is not coming from localhost, TRAVELOOP returns a 500 error.

**Fix:** Set `APP_URL` to the public URL of your instance:

```yaml
environment:
  - APP_URL=https://traveloop.example.com
```
...
**Cause:** TRAVELOOP validates that the `issuer` field in the provider's discovery document exactly matches the configured `OIDC_ISSUER`. A trailing-slash difference (e.g. `https://auth.example.com` vs `https://auth.example.com/`) is enough to fail.
...
**Cause:** TRAVELOOP's SSRF guard blocks outbound requests to private IP ranges by default. If your OIDC provider (e.g. Keycloak, Authentik) is running on an internal address, the discovery document fetch will be blocked with: `Requests to private/internal network addresses are not allowed.`
...
> **Note:** If no SMTP is configured at all, TRAVELOOP prints the reset link directly to the server logs (`===== PASSWORD RESET LINK =====`). This is useful for initial setup or self-hosted installs without email.
...
```yaml
environment:
  - ALLOWED_ORIGINS=https://traveloop.example.com,https://other.example.com
```

If `ALLOWED_ORIGINS` is not set, TRAVELOOP allows all origins (development default). See [Environment Variables](Environment-Variables).
...
**Cause:** The browser Clipboard API (`navigator.clipboard`) is only available in a [secure context](https://developer.mozilla.org/en-US/docs/Web/Security/Secure_Contexts). When accessing TRAVELOOP over plain HTTP on a non-localhost address, the API is unavailable and clipboard operations silently fail or show an error.

**Fix:** The only supported options are:

- Access TRAVELOOP over HTTPS with a valid SSL certificate.
- Access TRAVELOOP directly from `http://localhost:<port>` — browsers treat `localhost` as a secure context for the Clipboard API (unlike the session cookie, which always requires HTTPS regardless of hostname).

---

## MCP OAuth flow does not initiate / "Connect" redirects but authentication never starts

**Cause:** TRAVELOOP builds the OAuth 2.1 redirect URI from `APP_URL`. If `APP_URL` is not set, the authorization URL is constructed from a localhost fallback that external clients (Claude.ai, Claude Desktop) cannot reach, so the OAuth handshake never completes.
...
Restart the container after adding the variable. Once set, clicking **Connect** in the MCP client should redirect to your TRAVELOOP instance and complete the OAuth flow normally.
...
**Cause:** When TRAVELOOP is proxied through Cloudflare, **Bot Fight Mode** and **Super Bot Fight Mode** classify requests from ChatGPT as bots and block them at the WAF level — before the request ever reaches TRAVELOOP. This is specific to ChatGPT; Claude.ai is not affected. ChatGPT's exit node IPs have low reputation scores in Cloudflare's threat intelligence and the User-Agent matches Cloudflare's automated-traffic heuristics. TRAVELOOP itself never receives the request, so there is nothing in TRAVELOOP's logs; the block is silent from TRAVELOOP's perspective.
...
2. Enter the following expression (replace `traveloop.example.com` with your domain):

   ```
   (http.host eq "traveloop.example.com") and (
