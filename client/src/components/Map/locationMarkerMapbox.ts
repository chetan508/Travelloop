...
function buildLocationEl(): { root: HTMLDivElement; cone: HTMLDivElement } {
  ...
  pulse.style.cssText = `
    ...
    animation:traveloop-location-pulse 2s ease-out infinite;
  `
...
function ensurePulseStyle() {
  if (document.getElementById('traveloop-location-style')) return
  const s = document.createElement('style')
  s.id = 'traveloop-location-style'
  s.textContent = `
    @keyframes traveloop-location-pulse {
...
export function attachLocationMarker(map: mapboxgl.Map): LocationMarkerHandle {
  ...
  const ensureAccuracyLayer = () => {
    if (map.getSource('traveloop-location-accuracy')) return
    try {
      map.addSource('traveloop-location-accuracy', {
        type: 'geojson',
      ...
      map.addLayer({
        id: 'traveloop-location-accuracy',
        type: 'fill',
        source: 'traveloop-location-accuracy',
        paint: {
          'fill-color': '#3b82f6',
          'fill-opacity': 0.14,
  ...
  const setAccuracy = (p: GeoPosition) => {
    const src = map.getSource('traveloop-location-accuracy') as mapboxgl.GeoJSONSource | undefined
    if (!src) return
  ...
  const handle: LocationMarkerHandle = {
    ...
    destroy: () => {
      ...
      try {
        if (map.getLayer('traveloop-location-accuracy')) map.removeLayer('traveloop-location-accuracy')
        if (map.getSource('traveloop-location-accuracy')) map.removeSource('traveloop-location-accuracy')
      } catch { /* noop */ }
    },
  }
