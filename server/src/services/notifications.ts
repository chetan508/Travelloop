...
interface EmailStrings { footer: string; manage: string; madeWith: string; openTraveloop: string }

const I18N: Record<string, EmailStrings> = {
  en: { footer: 'You received this because you have notifications enabled in TRAVELOOP.', manage: 'Manage preferences in Settings', madeWith: 'Made with', openTraveloop: 'Open TRAVELOOP' },
  de: { footer: 'Du erhältst diese E-Mail, weil du Benachrichtigungen in TRAVELOOP aktiviert hast.', manage: 'Einstellungen verwalten', madeWith: 'Made with', openTraveloop: 'TRAVELOOP öffnen' },
  fr: { footer: 'Vous recevez cet e-mail car les notifications sont activées dans TRAVELOOP.', manage: 'Gérer les préférences', madeWith: 'Made with', openTraveloop: 'Ouvrir TRAVELOOP' },
  es: { footer: 'Recibiste esto porque tienes las notificaciones activadas en TRAVELOOP.', manage: 'Gestionar preferencias', madeWith: 'Made with', openTraveloop: 'Abrir TRAVELOOP' },
  nl: { footer: 'Je ontvangt dit omdat je meldingen hebt ingeschakeld in TRAVELOOP.', manage: 'Voorkeuren beheren', madeWith: 'Made with', openTraveloop: 'TRAVELOOP openen' },
  ru: { footer: 'Вы получили это, потому что у вас включены уведомления в TRAVELOOP.', manage: 'Управление настройками', madeWith: 'Made with', openTraveloop: 'Открыть TRAVELOOP' },
  zh: { footer: '您收到此邮件是因为您在 TRAVELOOP 中启用了通知。', manage: '管理偏好设置', madeWith: 'Made with', openTraveloop: '打开 TRAVELOOP' },
  'zh-TW': { footer: '您收到這封郵件是因為您在 TRAVELOOP 中啟用了通知。', manage: '管理偏好設定', madeWith: 'Made with', openTraveloop: '開啟 TRAVELOOP' },
  ar: { footer: 'تلقيت هذا لأنك قمت بتفعيل الإشعارات في TRAVELOOP.', manage: 'إدارة التفضيلات', madeWith: 'Made with', openTraveloop: 'فتح TRAVELOOP' },
  id: { footer: 'Anda menerima ini karena Anda telah mengaktifkan notifikasi di TRAVELOOP.', manage: 'Kelola preferensi di Pengaturan', madeWith: 'Dibuat dengan', openTraveloop: 'Buka TRAVELOOP' },
};
...
const EVENT_TEXTS: Record<string, Record<NotifEventType, EventTextFn>> = {
  en: {
    ...
    vacay_invite: p => ({ title: 'Vacay Fusion Invite', body: `${p.actor} invited you to fuse vacation plans. Open TRAVELOOP to accept or decline.` }),
    ...
    version_available: p => ({ title: 'New TRAVELOOP version available', body: `TRAVELOOP ${p.version} is now available. Visit the admin panel to update.` }),
  ...
  de: {
    ...
    vacay_invite: p => ({ title: 'Vacay Fusion-Einladung', body: `${p.actor} hat dich eingeladen, Urlaubspläne zu fusionieren. Öffne TRAVELOOP um anzunehmen oder abzulehnen.` }),
    ...
    version_available: p => ({ title: 'Neue TRAVELOOP-Version verfügbar', body: `TRAVELOOP ${p.version} ist jetzt verfügbar. Besuche das Admin-Panel zum Aktualisieren.` }),
  ...
  fr: {
    ...
    vacay_invite: p => ({ title: 'Invitation Vacay Fusion', body: `${p.actor} vous invite à fusionner les plans de vacances. Ouvrez TRAVELOOP pour accepter ou refuser.` }),
    ...
    version_available: p => ({ title: 'Nouvelle version TRAVELOOP disponible', body: `TRAVELOOP ${p.version} est maintenant disponible. Rendez-vous dans le panneau d'administration pour mettre à jour.` }),
  ...
  es: {
    ...
    vacay_invite: p => ({ title: 'Invitación Vacay Fusion', body: `${p.actor} te invitó a fusionar planes de vacaciones. Abre TRAVELOOP para aceptar o rechazar.` }),
    ...
    version_available: p => ({ title: 'Nueva versión de TRAVELOOP disponible', body: `TRAVELOOP ${p.version} ya está disponible. Visita el panel de administración para actualizar.` }),
  ...
  nl: {
    ...
    vacay_invite: p => ({ title: 'Vacay Fusion uitnodiging', body: `${p.actor} nodigt je uit om vakantieplannen te fuseren. Open TRAVELOOP om te accepteren of af te wijzen.` }),
    ...
    version_available: p => ({ title: 'Nieuwe TRAVELOOP-versie beschikbaar', body: `TRAVELOOP ${p.version} is nu beschikbaar. Bezoek het beheerderspaneel om bij te werken.` }),
  ...
  ru: {
    ...
    vacay_invite: p => ({ title: 'Приглашение Vacay Fusion', body: `${p.actor} приглашает вас объединить планы отпуска. Откройте TRAVELOOP для подтверждения.` }),
    ...
    version_available: p => ({ title: 'Доступна новая версия TRAVELOOP', body: `TRAVELOOP ${p.version} теперь доступен. Перейдите в панель администратора для обновления.` }),
  ...
  zh: {
    ...
    vacay_invite: p => ({ title: 'Vacay 融合邀请', body: `${p.actor} 邀请你合并假期计划。打开 TRAVELOOP 接受或拒绝。` }),
    ...
    version_available: p => ({ title: '新版 TRAVELOOP 可用', body: `TRAVELOOP ${p.version} 现已可用。请前往管理面板进行更新。` }),
  ...
  'zh-TW': {
    ...
    vacay_invite: p => ({ title: 'Vacay 融合邀請', body: `${p.actor} 邀請您合併假期計畫。開啟 TRAVELOOP 以接受或拒絕。` }),
    ...
    version_available: p => ({ title: '新版 TRAVELOOP 可用', body: `TRAVELOOP ${p.version} 現已可用。請前往管理面板進行更新。` }),
  ...
  ar: {
    ...
    vacay_invite: p => ({ title: 'دعوة دمج الإجازة', body: `${p.actor} يدعوك لدمج خطط الإجازة. افتح TRAVELOOP للقبول أو الرفض.` }),
    ...
    version_available: p => ({ title: 'إصدار TRAVELOOP جديد متاح', body: `TRAVELOOP ${p.version} متاح الآن. تفضل بزيارة لوحة الإدارة للتحديث.` }),
  ...
  br: {
    ...
    vacay_invite: p => ({ title: 'Convite Vacay Fusion', body: `${p.actor} convidou você para fundir planos de férias. Abra o TRAVELOOP para aceitar ou recusar.` }),
    ...
    version_available: p => ({ title: 'Nova versão do TRAVELOOP disponível', body: `O TRAVELOOP ${p.version} está disponível. Acesse o painel de administração para atualizar.` }),
  ...
  cs: {
    ...
    vacay_invite: p => ({ title: 'Pozvánka Vacay Fusion', body: `${p.actor} vás pozval ke spojení dovolenkových plánů. Otevřete TRAVELOOP pro přijetí nebo odmítnutí.` }),
    ...
    version_available: p => ({ title: 'Nová verze TRAVELOOP dostupná', body: `TRAVELOOP ${p.version} je nyní dostupný. Navštivte administrátorský panel pro aktualizaci.` }),
  ...
  hu: {
    ...
    vacay_invite: p => ({ title: 'Vacay Fusion meghívó', body: `${p.actor} meghívott a nyaralási tervek összevonásához. Nyissa meg a TRAVELOOP-et az elfogadáshoz vagy elutasításhoz.` }),
    ...
    version_available: p => ({ title: 'Új TRAVELOOP verzió érhető el', body: `A TRAVELOOP ${p.version} elérhető. Látogasson el az adminisztrációs panelre a frissítéshez.` }),
  ...
  it: {
    ...
    vacay_invite: p => ({ title: 'Invito Vacay Fusion', body: `${p.actor} ti ha invitato a fondere i piani vacanza. Apri TRAVELOOP per accettare o rifiutare.` }),
    ...
    version_available: p => ({ title: 'Nuova versione TRAVELOOP disponibile', body: `TRAVELOOP ${p.version} è ora disponibile. Visita il pannello di amministrazione per aggiornare.` }),
  ...
  pl: {
    ...
    vacay_invite: p => ({ title: 'Zaproszenie Vacay Fusion', body: `${p.actor} zaprosił Cię do połączenia planów urlopowych. Otwórz TRAVELOOP, aby zaakceptować lub odrzucić.` }),
    ...
    version_available: p => ({ title: 'Nowa wersja TRAVELOOP dostępna', body: `TRAVELOOP ${p.version} jest teraz dostępny. Odwiedź panel administracyjny, aby zaktualizować.` }),
  ...
  id: {
    ...
    vacay_invite: p => ({ title: 'Undangan Penggabungan Vacay', body: `${p.actor} mengundang Anda untuk menggabungkan rencana liburan. Buka TRAVELOOP untuk menerima atau menolak.` }),
    ...
    version_available: p => ({ title: 'Versi TRAVELOOP baru tersedia', body: `TRAVELOOP ${p.version} sekarang tersedia. Kunjungi panel admin untuk memperbarui.` }),
  },
};
    ...
    <tr><td align="center">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width: 480px; background: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 24px rgba(0,0,0,0.06);">
        ...
        ${appUrl ? `<tr><td style="padding: 8px 32px 32px; text-align: center;">
          <a href="${ctaHref}" style="display: inline-block; padding: 12px 28px; background: #111827; color: #ffffff; font-size: 13px; font-weight: 600; text-decoration: none; border-radius: 10px; letter-spacing: 0.2px;">${s.openTraveloop}</a>
        </td></tr>` : ''}
        <!-- Footer -->
        <tr><td style="padding: 20px 32px; background: #f9fafb; border-top: 1px solid #f3f4f6; text-align: center;">
          ...
          <p style="margin: 0; font-size: 10px; color: #d1d5db;">${s.madeWith} <span style="color: #ef4444;">&hearts;</span> by Maurice &middot; <a href="https://github.com/mauriceboe/TRAVELOOP" style="color: #9ca3af; text-decoration: none;">GitHub</a></p>
        </td></tr>
      </table>
    </td></tr>
  </table>
...
const PASSWORD_RESET_I18N: Record<string, PasswordResetStrings> = {
  en: { subject: 'Reset your password', greeting: 'Hi', body: 'We received a request to reset the password for your TRAVELOOP account. Click the button below to set a new password.', ctaIntro: 'Reset password', expiry: 'This link expires in 60 minutes.', ignore: "If you didn't request this, you can safely ignore this email — your password won't change." },
  de: { subject: 'Passwort zurücksetzen', greeting: 'Hallo', body: 'Wir haben eine Anfrage erhalten, das Passwort für dein TRAVELOOP-Konto zurückzusetzen. Klicke auf den Button unten, um ein neues Passwort festzulegen.', ctaIntro: 'Passwort zurücksetzen', expiry: 'Dieser Link ist 60 Minuten gültig.', ignore: 'Wenn du das nicht warst, ignoriere diese E-Mail — dein Passwort bleibt unverändert.' },
  fr: { subject: 'Réinitialisez votre mot de passe', greeting: 'Bonjour', body: 'Nous avons reçu une demande de réinitialisation du mot de passe de votre compte TRAVELOOP. Cliquez sur le bouton ci-dessous pour définir un nouveau mot de passe.', ctaIntro: 'Réinitialiser le mot de passe', expiry: 'Ce lien expire dans 60 minutes.', ignore: "Si vous n'êtes pas à l'origine de cette demande, ignorez cet e-mail — votre mot de passe ne changera pas." },
  es: { subject: 'Restablecer tu contraseña', greeting: 'Hola', body: 'Recibimos una solicitud para restablecer la contraseña de tu cuenta de TRAVELOOP. Haz clic en el botón de abajo para establecer una nueva contraseña.', ctaIntro: 'Restablecer contraseña', expiry: 'Este enlace caduca en 60 minutos.', ignore: 'Si no solicitaste esto, puedes ignorar este correo — tu contraseña no cambiará.' },
  it: { subject: 'Reimposta la tua password', greeting: 'Ciao', body: 'Abbiamo ricevuto una richiesta di reimpostazione della password per il tuo account TRAVELOOP. Clicca il pulsante qui sotto per impostare una nuova password.', ctaIntro: 'Reimposta password', expiry: 'Questo link scade tra 60 minuti.', ignore: 'Se non hai richiesto questa operazione, ignora questa email — la tua password non cambierà.' },
  nl: { subject: 'Reset je wachtwoord', greeting: 'Hallo', body: 'We hebben een verzoek ontvangen om het wachtwoord voor je TRAVELOOP-account te resetten. Klik op de knop hieronder om een nieuw wachtwoord in te stellen.', ctaIntro: 'Wachtwoord resetten', expiry: 'Deze link verloopt over 60 minuten.', ignore: 'Als jij dit niet hebt aangevraagd, kun je deze e-mail negeren — je wachtwoord blijft ongewijzigd.' },
  ru: { subject: 'Сброс пароля', greeting: 'Здравствуйте', body: 'Мы получили запрос на сброс пароля вашего аккаунта TRAVELOOP. Нажмите кнопку ниже, чтобы установить новый пароль.', ctaIntro: 'Сбросить пароль', expiry: 'Ссылка действительна 60 минут.', ignore: 'Если вы не запрашивали сброс — просто проигнорируйте это письмо, пароль останется прежним.' },
  zh: { subject: '重置您的密码', greeting: '您好', body: '我们收到了重置您的 TRAVELOOP 账户密码的请求。点击下方按钮设置新密码。', ctaIntro: '重置密码', expiry: '此链接将在 60 分钟后失效。', ignore: '如果这不是您本人的请求，可以忽略本邮件 — 您的密码不会改变。' },
  'zh-TW': { subject: '重設您的密碼', greeting: '您好', body: '我們收到了重設您 TRAVELOOP 帳號密碼的請求。點擊下方按鈕以設定新密碼。', ctaIntro: '重設密碼', expiry: '此連結將於 60 分鐘後失效。', ignore: '若非您本人發起的請求，請忽略此郵件 — 您的密碼不會變更。' },
  hu: { subject: 'Jelszó visszaállítása', greeting: 'Szia', body: 'Kérést kaptunk a TRAVELOOP-fiókod jelszavának visszaállítására. Kattints az alábbi gombra az új jelszó beállításához.', ctaIntro: 'Jelszó visszaállítása', expiry: 'Ez a link 60 perc után lejár.', ignore: 'Ha nem te kérted ezt, nyugodtan hagyd figyelmen kívül ezt az e-mailt — a jelszavad változatlan marad.' },
  ar: { subject: 'إعادة تعيين كلمة المرور', greeting: 'مرحبا', body: 'تلقينا طلبًا لإعادة تعيين كلمة المرور لحسابك في TRAVELOOP. انقر على الزر أدناه لتعيين كلمة مرور جديدة.', ctaIntro: 'إعادة تعيين كلمة المرور', expiry: 'تنتهي صلاحية هذا الرابط خلال 60 دقيقة.', ignore: 'إذا لم تطلب هذا، يمكنك تجاهل هذه الرسالة — لن تتغير كلمة المرور الخاصة بك.' },
  br: { subject: 'Redefinir sua senha', greeting: 'Olá', body: 'Recebemos um pedido para redefinir a senha da sua conta TRAVELOOP. Clique no botão abaixo para definir uma nova senha.', ctaIntro: 'Redefinir senha', expiry: 'Este link expira em 60 minutos.', ignore: 'Se você não solicitou isto, pode ignorar este e-mail — sua senha não será alterada.' },
  cs: { subject: 'Obnovení hesla', greeting: 'Ahoj', body: 'Obdrželi jsme žádost o obnovení hesla k tvému účtu TRAVELOOP. Klikni na tlačítko níže a nastav nové heslo.', ctaIntro: 'Obnovit heslo', expiry: 'Odkaz vyprší za 60 minut.', ignore: 'Pokud jsi o obnovení nežádal/a, tento e-mail ignoruj — heslo zůstane beze změny.' },
  pl: { subject: 'Zresetuj hasło', greeting: 'Cześć', body: 'Otrzymaliśmy prośbę o zresetowanie hasła do Twojego konta TRAVELOOP. Kliknij przycisk poniżej, aby ustawić nowe hasło.', ctaIntro: 'Zresetuj hasło', expiry: 'Link wygaśnie za 60 minut.', ignore: 'Jeśli to nie Ty, zignoruj tę wiadomość — Twoje hasło pozostanie bez zmian.' },
};
...
export async function sendPasswordResetEmail(
  ...
  try {
    ...
    await transporter.sendMail({
      from: smtpCfg.from,
      to,
      subject: `TRAVELOOP — ${strings.subject}`,
...
export function buildWebhookBody(url: string, payload: { event: string; title: string; body: string; tripName?: string; link?: string }): string {
  ...
  if (isDiscord) {
    return JSON.stringify({
      embeds: [{
        ...
        url: payload.link,
        color: 0x3b82f6,
        footer: { text: payload.tripName ? `Trip: ${payload.tripName}` : 'TRAVELOOP' },
  ...
  if (isSlack) {
    ...
    const link = payload.link ? `\n<${payload.link}|Open in TRAVELOOP>` : '';
    return JSON.stringify({
      text: `*${payload.title}*\n${payload.body}${trip}${link}`,
    });
  }

  return JSON.stringify({ ...payload, timestamp: new Date().toISOString(), source: 'TRAVELOOP' });
}
...
export async function testSmtp(to: string): Promise<{ success: boolean; error?: string }> {
  ...
  try {
    ...
    await transporter.sendMail({
      from: config.from,
      to,
      subject: 'TRAVELOOP — Test Notification',
      text: 'This is a test email from TRAVELOOP. If you received this, your SMTP configuration is working correctly.',
    });
    return { success: true };
  } catch (err) {
...
export async function testWebhook(url: string): Promise<{ success: boolean; error?: string }> {
  try {
    const sent = await sendWebhook(url, { event: 'test', title: 'Test Notification', body: 'This is a test webhook from TRAVELOOP. If you received this, your webhook configuration is working correctly.' });
...
export async function testNtfy(cfg: { topic: string; server?: string | null; token?: string | null }): Promise<{ success: boolean; error?: string }> {
  ...
  try {
    const sent = await sendNtfy(url, cfg.token ?? null, {
      ...
      body: 'This is a test notification from TRAVELOOP. If you received this, your ntfy configuration is working correctly.',
    });
