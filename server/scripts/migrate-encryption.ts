 ...
 *
 * Re-encrypts all at-rest secrets in the TRAVELOOP database from one ENCRYPTION_KEY
...
async function main() {
  console.log('=== TRAVELOOP Encryption Key Migration ===\n');
  ...
  db.transaction(() => {
    ...
    const photos = db.prepare('SELECT id, passphrase FROM traveloop_photos WHERE passphrase IS NOT NULL').all() as { id: number; passphrase: string }[];
    for (const row of photos) {
      const newVal = migrateApiKeyValue(row.passphrase, `traveloop_photos[${row.id}].passphrase`);
      if (newVal !== null) {
        db.prepare('UPDATE traveloop_photos SET passphrase = ? WHERE id = ?').run(newVal, row.id);
      }
    }
  })();
