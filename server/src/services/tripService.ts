...
export function exportICS(tripId: string | number): { ics: string; filename: string } {
  ...
  const uid = (id: number, type: string) => `traveloop-${type}-${id}@traveloop`;
  ...
  let ics = 'BEGIN:VCALENDAR\r\nVERSION:2.0\r\nPRODID:-//TRAVELOOP//Travel Planner//EN\r\nCALSCALE:GREGORIAN\r\nMETHOD:PUBLISH\r\n';
  ics += `X-WR-CALNAME:${esc(trip.title || 'TRAVELOOP Trip')}\r\n`;
  ...
  const safeFilename = (trip.title || 'traveloop-trip').replace(/["\r\n]/g, '').replace(/[^\w\s.-]/g, '_');
