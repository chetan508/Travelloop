...
See https://github.com/mauriceboe/TRAVELOOP/blob/main/docker-compose.yml
...
services:
  app:
    ...
    volumes:
      - traveloop_data:/app/data
      - traveloop_uploads:/app/uploads
...
```yaml
image: mauriceboe/traveloop:3        # track major version 3
image: mauriceboe/traveloop:3.0.15   # pin to exact release
```
...
This compose file is designed for deployments where a reverse proxy (nginx, Caddy, Traefik) terminates TLS in front of TRAVELOOP. To enable HTTPS redirects and secure cookies, uncomment `FORCE_HTTPS=true` and `TRUST_PROXY=1`.
