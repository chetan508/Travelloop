...
You are connected to TRAVELOOP, a travel planning application. Below is a compact reference of the data model, key workflows, and behavioral rules you must follow.
...
The following features are optional and may not be available on every TRAVELOOP instance. Check tool availability before assuming they exist:
...
const STATIC_TOKEN_DEPRECATION_NOTICE =
    ...
    '"Your TRAVELOOP integration is using a static API token that is deprecated and will stop working in a future version. ' +
    'Please migrate to OAuth 2.1: go to Settings → Integrations → MCP → OAuth Clients in TRAVELOOP and register an OAuth 2.1 application." ' +
...
function setAuthChallenge(res: Response, error = 'invalid_token'): void {
  ...
  res.set('WWW-Authenticate',
      `Bearer realm="TRAVELOOP MCP", resource_metadata="${base}/.well-known/oauth-protected-resource/mcp", error="${error}"`);
}
...
function verifyToken(authHeader: string | undefined): VerifyTokenResult | null {
  ...
  // OAuth 2.1 access token (traveloopoa_...)
  if (token.startsWith('traveloopoa_')) {
  ...
  // Long-lived static MCP token (traveloop_...) — full access + deprecation notice
  if (token.startsWith('traveloop_')) {
    const user = verifyMcpToken(token);
    if (!user) return null;
    return { user, scopes: null, clientId: null, isStaticToken: true };
  }

  // Short-lived JWT (TRAVELOOP web session used directly) — full access, no notice
