...
import type { TraveloopPhoto } from '../../types';
...
import * as photoCache from './traveloopPhotoCache';
import { ensureLocalThumbnail } from './thumbnailService';

// ── Lookup / Register ────────────────────────────────────────────────────

export function getOrCreateTraveloopPhoto(
  provider: string,
  assetId: string,
  ownerId: number,
  passphrase?: string,
  ...
  const existing = db.prepare(
    'SELECT id FROM traveloop_photos WHERE provider = ? AND asset_id = ? AND owner_id = ?'
  ).get(provider, assetId, ownerId) as { id: number } | undefined;
  if (existing) {
    if (passphrase) {
      db.prepare('UPDATE traveloop_photos SET passphrase = ? WHERE id = ?')
  ...
  const res = db.prepare(
    'INSERT INTO traveloop_photos (provider, asset_id, owner_id, passphrase) VALUES (?, ?, ?, ?)'
  ).run(provider, assetId, ownerId, passphrase ? encrypt_api_key(passphrase) : null);
  return Number(res.lastInsertRowid);
}

export function getOrCreateLocalTraveloopPhoto(
  filePath: string,
  ...
  const existing = db.prepare(
    "SELECT id FROM traveloop_photos WHERE provider = 'local' AND file_path = ?"
  ).get(filePath) as { id: number } | undefined;
  if (existing) return existing.id;

  const res = db.prepare(
    'INSERT INTO traveloop_photos (provider, file_path, thumbnail_path, width, height) VALUES (?, ?, ?, ?, ?)'
  ).run('local', filePath, thumbnailPath || null, width || null, height || null);
  return Number(res.lastInsertRowid);
}

export function resolveTraveloopPhoto(photoId: number): TraveloopPhoto | null {
  return db.prepare('SELECT * FROM traveloop_photos WHERE id = ?').get(photoId) as TraveloopPhoto | undefined || null;
}
...
export async function streamPhoto(
  ...
  const photo = resolveTraveloopPhoto(photoId);
  if (!photo) {
    ...
    if (kind === 'thumbnail') {
      ...
      if (!thumbRel) {
        ...
        if (result) {
          ...
          db.prepare(
            'UPDATE traveloop_photos SET thumbnail_path = ?, width = COALESCE(width, ?), height = COALESCE(height, ?) WHERE id = ?'
...
// ── Update provider on existing traveloop_photo (for Immich upload sync) ─────

export function setTraveloopPhotoProvider(
  traveloopPhotoId: number,
  provider: string,
  assetId: string,
  ownerId: number,
): void {
  db.prepare(
    'UPDATE traveloop_photos SET provider = ?, asset_id = ?, owner_id = ? WHERE id = ?'
  ).run(provider, assetId, ownerId, traveloopPhotoId);
}

// ── Orphan cleanup ───────────────────────────────────────────────────────

export function deleteTraveloopPhotoIfOrphan(photoId: number): void {
  ...
  `).get(photoId, photoId);
  if (stillUsed) return;
  db.prepare("DELETE FROM traveloop_photos WHERE id = ? AND provider != 'local'").run(photoId);
}

// ── Delete local file for a traveloop_photo ──────────────────────────────────

export function getTraveloopPhotoFilePath(photoId: number): string | null {
  const photo = resolveTraveloopPhoto(photoId);
