...
Putting TRAVELOOP behind a TLS-terminating reverse proxy is strongly recommended for production.

## Why HTTPS Matters for TRAVELOOP

- **PWA install** requires HTTPS — browsers block "Add to Home Screen" on plain HTTP.
- **Session cookies** — the `traveloop_session` cookie is marked `secure` in production, so it won't be sent over HTTP.
...
1. **WebSocket upgrades on `/ws`** — TRAVELOOP uses WebSockets for real-time sync. Set `proxy_read_timeout 86400` (Nginx) or rely on Caddy's automatic upgrade handling.
...
- `X-Forwarded-Proto $scheme` — tells TRAVELOOP whether the original request was HTTPS; required for `FORCE_HTTPS` redirect and cookie security to work correctly.
...
Four variables control how TRAVELOOP behaves behind a proxy. They work as a group:
...
| `COOKIE_SECURE` | Controls the `secure` flag on `traveloop_session`. Auto-derived as `true` when `NODE_ENV=production` or `FORCE_HTTPS=true`. Set to `false` explicitly to allow cookies over plain HTTP (e.g. LAN testing without TLS). | auto |
| `ALLOWED_ORIGINS` | Comma-separated list of allowed CORS origins (e.g. `https://traveloop.example.com`). In production without this set, all cross-origin requests are blocked. In development without this set, all origins are allowed. | blocked in prod, open in dev |
...
If you access TRAVELOOP directly on `http://<host>:3000` without a proxy, leave `FORCE_HTTPS` unset and do not set `TRUST_PROXY`.
