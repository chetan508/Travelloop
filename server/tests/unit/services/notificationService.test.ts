...
function setSmtp(): void {
  ...
  setAppSetting(testDb, 'smtp_from', 'traveloop@test.com');
}
...
function setUserNtfyTopic(userId: number, topic = 'my-traveloop-topic'): void {
  testDb.prepare("INSERT OR REPLACE INTO settings (user_id, key, value) VALUES (?, 'ntfy_topic', ?)").run(userId, topic);
}

function setAdminNtfyTopic(topic = 'traveloop-admin-alerts'): void {
