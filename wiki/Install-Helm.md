...
Deploy TRAVELOOP on Kubernetes using the official Helm chart.

## Add the Chart Repository

```bash
helm repo add traveloop https://mauriceboe.github.io/TRAVELOOP
helm repo update
```
...
```bash
helm install traveloop traveloop/traveloop
```

This deploys TRAVELOOP with default values: a `ClusterIP` service on port 3000, 1 Gi PVCs for data and uploads, and no ingress.
...
```bash
helm install traveloop traveloop/traveloop --set generateEncryptionKey=true
```
...
```bash
kubectl create secret generic traveloop-secrets \
  --from-literal=ENCRYPTION_KEY=$(openssl rand -hex 32)

helm install traveloop traveloop/traveloop \
  --set existingSecret=traveloop-secrets
```
...
```yaml
image:
  repository: mauriceboe/traveloop
...
env:
  ...
  # ALLOWED_ORIGINS: "https://traveloop.example.com"
...
ingress:
  ...
  hosts:
    - host: traveloop.example.com
      paths:
        - /
  tls:
    - secretName: traveloop-tls
      hosts:
        - traveloop.example.com
```

> **Important:** TRAVELOOP uses WebSockets on `/ws`. Your ingress controller must support WebSocket upgrades. Set `proxy-read-timeout` to at least `86400` and `proxy-body-size` to at least `500m` for backup restores.
...
```bash
helm repo update
helm upgrade traveloop traveloop/traveloop
```

## Full Values Reference

See the [`charts/README.md`](https://github.com/mauriceboe/TRAVELOOP/blob/main/charts/README.md) for all available values.
