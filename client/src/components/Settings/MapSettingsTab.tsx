...
const FALLBACK_MAP_TILE_URL = MAP_PRESETS[2].url
...
> {
  ...
  componentDidCatch(error: unknown) {
    console.error('Map settings preview failed', error)
  }
...
export default function MapSettingsTab(): React.ReactElement {
  ...
  const previewLat = parseFloat(String(defaultLat)) || 48.8566
  const previewLng = parseFloat(String(defaultLng)) || 2.3522
  const previewZoom = parseInt(String(defaultZoom)) || 10
  const previewTileUrl = mapTileUrl.trim() || FALLBACK_MAP_TILE_URL
    ...
    <Section title={t('settings.map')} icon={Map}>
      ...
      <div>
        <div style={{ position: 'relative', inset: 0, height: '200px', width: '100%' }}>
          <PreviewErrorBoundary key={`${provider}:${previewLat}:${previewLng}:${previewZoom}:${provider === 'leaflet' ? previewTileUrl : mapboxStyle}`}>
            {provider === 'mapbox-gl' ? (
              <MapboxPreview
                token={mapboxToken}
                style={mapboxStyle}
