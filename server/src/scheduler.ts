...
// Version check: daily at 9 AM — notify admins if a new TRAVELOOP release is available
...
// Traveloop photo cache cleanup: every 2 hours — evict disk files and DB rows past their 1h TTL
let traveloopPhotoCacheTask: ScheduledTask | null = null;

function startTraveloopPhotoCacheCleanup(): void {
  if (traveloopPhotoCacheTask) { traveloopPhotoCacheTask.stop(); traveloopPhotoCacheTask = null; }

  // Run once immediately on startup to evict any entries left over from a previous run
  try {
    const { sweepExpired } = require('./services/memories/traveloopPhotoCache');
    sweepExpired();
  } catch { /* cache dir may not exist yet — harmless */ }

  traveloopPhotoCacheTask = cron.schedule('0 */2 * * *', () => {
    try {
      const { sweepExpired } = require('./services/memories/traveloopPhotoCache');
      sweepExpired();
    } catch (err: unknown) {
      logError(`Traveloop photo cache cleanup: ${err instanceof Error ? err.message : err}`);
    }
  });
}
...
export { start, stop, startDemoReset, startTripReminders, startTodoReminders, startVersionCheck, startIdempotencyCleanup, startTraveloopPhotoCacheCleanup, loadSettings, saveSettings, VALID_INTERVALS };
