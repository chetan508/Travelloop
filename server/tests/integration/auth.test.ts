...
describe('Login', () => {
  it('AUTH-001 — successful login returns 200, user object, and traveloop_session cookie', async () => {
    ...
    const cookies: string[] = Array.isArray(res.headers['set-cookie'])
      ...
      : [res.headers['set-cookie']];
    expect(cookies.some((c: string) => c.includes('traveloop_session'))).toBe(true);
  });
  ...
  it('AUTH-013 — POST /api/auth/logout clears session cookie', async () => {
    ...
    const sessionCookie = cookies.find((c: string) => c.includes('traveloop_session'));
...
describe('Demo login', () => {
  ...
  it('AUTH-022 — POST /api/auth/demo-login with DEMO_MODE and demo user returns 200 + cookie', async () => {
    testDb.prepare(
      "INSERT INTO users (username, email, password_hash, role) VALUES ('demo', 'demo@traveloop.app', 'x', 'user')"
    ).run();
    process.env.DEMO_MODE = 'true';
    try {
      const res = await request(app).post('/api/auth/demo-login');
      expect(res.status).toBe(200);
      expect(res.body.user.email).toBe('demo@traveloop.app');
    } finally {
      delete process.env.DEMO_MODE;
    }
  });
...
describe('Account deletion', () => {
  it('AUTH-040 — DELETE /auth/me succeeds when user has FK references', async () => {
    ...
    // traveloop_photos.owner_id (SET NULL): target owns a photo in the central registry
    const traveloopPhotoRow = testDb.prepare(
      "INSERT INTO traveloop_photos (provider, asset_id, owner_id) VALUES ('immich', 'asset-auth-test', ?)"
    ).run(target.id);
    ...
    // traveloop_photos row survives but owner_id is now NULL
    expect((testDb.prepare('SELECT owner_id FROM traveloop_photos WHERE id = ?').get(traveloopPhotoRow.lastInsertRowid) as any).owner_id).toBeNull();
