...
OpenID Connect (OIDC) lets users log in with an existing identity provider — Google, Authentik, Keycloak, or any OIDC-compatible IdP — instead of a local email/password. On first SSO login, a TRAVELOOP account is created automatically using the email from the provider.
...
4. The provider redirects back to TRAVELOOP at `GET /api/auth/oidc/callback`. If this is your first login, an account is created automatically (subject to registration settings).
...
6. Your `traveloop_session` cookie is set and you land on the dashboard.
...
|---|---|---|
| `APP_URL` | Yes | Base URL of your TRAVELOOP instance (e.g. `https://traveloop.example.com`). Used to build the redirect URI. Can also be set via Admin → Settings. |
...
For example: `https://traveloop.example.com/api/auth/oidc/callback`
...
When an SSO login matches an existing TRAVELOOP account by email or OIDC subject (`sub`), that account is used and the OIDC identity is linked automatically. If no matching account exists, TRAVELOOP attempts to create one. The outcome depends on the following:
