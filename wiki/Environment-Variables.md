...
Complete reference for all environment variables TRAVELOOP reads.
...
| `APP_URL` | Public base URL (e.g. `https://traveloop.example.com`). Required when OIDC is enabled — must match the redirect URI registered with your IdP. Also used as the base URL for email notification links. | — |

### `HOST` — Source and Proxmox installs only

By default TRAVELOOP binds to all network interfaces (`0.0.0.0`), which is the correct behaviour inside a container because Docker handles port exposure at the host level. Setting `HOST` overrides the bind address at the Node.js level.

**When to use it:** only when running TRAVELOOP directly on a host (git sources or the [Proxmox community script](Install-Proxmox)) and you need to restrict which interface the server listens on — for example, to expose TRAVELOOP only on a LAN interface while keeping it off the public-facing one.
...
| `COOKIE_SECURE` | Controls the `secure` flag on the `traveloop_session` cookie. Auto-derived as `true` when `NODE_ENV=production` or `FORCE_HTTPS=true`. Set to `false` only as an escape hatch for LAN testing without TLS — not recommended in production. | auto |
...
| `OIDC_ADMIN_VALUE` | Value of the OIDC claim that grants admin role (e.g. `app-traveloop-admins`) | — |
...
| `SMTP_FROM` | Sender address for outbound emails (e.g. `TRAVELOOP <noreply@example.com>`) | — |
...
| Variable | Description | Default |
|---|---|---|
| `ADMIN_EMAIL` | Email for the first admin account | `admin@traveloop.local` |
| `ADMIN_PASSWORD` | Password for the first admin account | random |

Both variables must be set together. If either is omitted, the account is created with email `admin@traveloop.local` and a randomly generated password that is printed to the server log. Once any user exists, these variables have no effect.
