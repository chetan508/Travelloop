...
import { streamPhoto, getPhotoInfo, resolveTraveloopPhoto } from '../services/memories/photoResolverService';
import { canAccessTraveloopPhoto } from '../services/memories/helpersService';
...
router.get('/:id/thumbnail', authenticate, async (req: Request, res: Response) => {
  ...
  if (!canAccessTraveloopPhoto(authReq.user.id, photoId)) {
