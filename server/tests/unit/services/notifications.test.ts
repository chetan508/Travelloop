...
describe('buildWebhookBody', () => {
  ...
  it('Discord embed footer defaults to TRAVELOOP when no trip name', () => {
    ...
    expect(body.embeds[0].footer.text).toBe('TRAVELOOP');
  });
  ...
  it('generic URL produces plain JSON with original fields plus timestamp and source', () => {
    ...
    expect(body).toHaveProperty('source', 'TRAVELOOP');
  });
});
...
describe('buildEmailHtml', () => {
  ...
  it('uses English i18n strings for lang=en', () => {
    ...
    expect(html).toContain('notifications enabled in TRAVELOOP');
  });

  it('uses German i18n strings for lang=de', () => {
    const html = buildEmailHtml('Subject', 'Body', 'de');
    expect(html).toContain('TRAVELOOP aktiviert');
  });
  ...
  it('falls back to English i18n for unknown language', () => {
    ...
    expect(unknown).toContain('notifications enabled in TRAVELOOP');
  });
});
...
describe('sendNtfy', () => {
  const ntfyUrl = 'https://ntfy.sh/traveloop-test';
  ...
  it('NTFY-004 — includes Click header when link is provided', async () => {
    ...
    await sendNtfy(ntfyUrl, null, { ...payload, link: 'https://traveloop.example.com/trips/5' });

    const [, calledOpts] = mockFetch.mock.calls[0];
    expect(calledOpts.headers['Click']).toBe('https://traveloop.example.com/trips/5');
  });
