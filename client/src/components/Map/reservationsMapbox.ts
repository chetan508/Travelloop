...
export const RESERVATION_SOURCE_ID = 'traveloop-reservations'
export const RESERVATION_LINE_LAYER_ID = 'traveloop-reservations-lines'
...
function buildStatsHtml(mainLabel: string | null, subLabel: string | null): { html: string; width: number; height: number } {
  ...
  const html = `<div class="traveloop-stats-inner" style="
  ...
  private updateStatsRotation = () => {
    ...
    for (const entry of this.statsMarkers) {
      ...
      try {
        ...
        const inner = el.querySelector('.traveloop-stats-inner') as HTMLElement | null
