...
How to update TRAVELOOP to a newer version without losing data.
...
|---|---|---|
| `latest` | `mauriceboe/traveloop:latest` | Always the newest release across all major versions |
| Major version | `mauriceboe/traveloop:3` | Latest release pinned to that major version |
| Full version | `mauriceboe/traveloop:3.0.15` | Exact release; never changes |
...
If you started TRAVELOOP with `docker run`, pull the new image and replace the container:

```bash
docker pull mauriceboe/traveloop
docker rm -f traveloop
docker run -d --name traveloop -p 3000:3000 \
  -v ./data:/app/data \
  -v ./uploads:/app/uploads \
  -e ENCRYPTION_KEY=<your-key> \
  --restart unless-stopped \
  mauriceboe/traveloop
```

> **Tip:** Not sure which volume paths you used? Check before removing:
> ```bash
> docker inspect traveloop --format '{{json .Mounts}}'
> ```

## Database Migrations

TRAVELOOP runs any pending database migrations automatically at startup. No manual migration steps are required after pulling a new image.

## Encryption Key Note

If you are upgrading from a version that predates the dedicated `ENCRYPTION_KEY` (i.e. you have no `ENCRYPTION_KEY` environment variable set), TRAVELOOP automatically falls back to `./data/.jwt_secret` on startup and immediately promotes it to `./data/.encryption_key`. No manual steps are required — the transition is handled at first boot after the upgrade.
...
If you installed TRAVELOOP via the [Proxmox VE Community Scripts](https://community-scripts.org/scripts/traveloop), run the following command inside the **LXC container** and select **Update** when prompted:
...
Open the **Stacks** list, click the TRAVELOOP stack, then click **Redeploy**.
...
In the Unraid Docker tab, click the TRAVELOOP container and select **Update**. Unraid will pull the latest image and restart with the same volumes.
