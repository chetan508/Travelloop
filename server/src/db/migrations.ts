...
function runMigrations(db: Database.Database): void {
  ...
  const migrations: Migration[] = [
    ...
    // Central traveloop_photos registry; trip_photos + journey_photos reference via photo_id
    () => {
      // 1. Create the central photo registry
      db.exec(`
        CREATE TABLE IF NOT EXISTS traveloop_photos (
          id INTEGER PRIMARY KEY AUTOINCREMENT,
          provider TEXT NOT NULL,
          asset_id TEXT,
      ...
      `);
      db.exec('CREATE UNIQUE INDEX IF NOT EXISTS idx_traveloop_photos_provider_asset ON traveloop_photos(provider, asset_id, owner_id) WHERE asset_id IS NOT NULL');
      db.exec('CREATE INDEX IF NOT EXISTS idx_traveloop_photos_owner ON traveloop_photos(owner_id)');

      // 2. Migrate trip_photos → traveloop_photos + photo_id FK
      const tripPhotosExists = db.prepare("SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = 'trip_photos'").get();
      if (tripPhotosExists) {
        ...
        if (assetCol) {
          ...
          // Qualified alias needed in JOIN context where both trip_photos and traveloop_photos have provider
          ...
          // Insert existing trip photo references into traveloop_photos
          db.exec(`
            INSERT OR IGNORE INTO traveloop_photos (provider, asset_id, owner_id, created_at)
          ...
          db.exec(`
            CREATE TABLE trip_photos_new (
              ...
              trip_id INTEGER NOT NULL REFERENCES trips(id) ON DELETE CASCADE,
              user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
              photo_id INTEGER NOT NULL REFERENCES traveloop_photos(id) ON DELETE CASCADE,
              shared INTEGER NOT NULL DEFAULT 1,
          ...
          db.exec(`
            ...
            FROM trip_photos tp
            JOIN traveloop_photos tkp ON tkp.provider = ${providerJoinExpr} AND tkp.asset_id = tp.${assetCol} AND tkp.owner_id = tp.user_id
          `);
        } else {
      ...
      // 3. Migrate journey_photos → traveloop_photos + photo_id FK
      ...
      if (journeyPhotosExists) {
        // Insert provider-based journey photos into traveloop_photos
        db.exec(`
          INSERT OR IGNORE INTO traveloop_photos (provider, asset_id, owner_id, width, height, created_at)
        ...
        `);
        // Insert local journey photos into traveloop_photos (each is unique)
        db.exec(`
          INSERT INTO traveloop_photos (provider, file_path, thumbnail_path, width, height, created_at)
        ...
        db.exec(`
          ...
          JOIN traveloop_photos tkp ON tkp.provider = jp.provider AND tkp.asset_id = jp.asset_id AND tkp.owner_id = jp.owner_id
        ...
        db.exec(`
          ...
          SELECT jp.entry_id, tkp.id, jp.caption, jp.sort_order, jp.shared, jp.created_at
          FROM journey_photos jp
          JOIN traveloop_photos tkp ON tkp.provider = 'local' AND tkp.file_path = jp.file_path
    ...
    () => {
      ...
      try { db.exec('ALTER TABLE traveloop_photos ADD COLUMN passphrase TEXT DEFAULT NULL'); } catch (err: any) { if (!err.message?.includes('duplicate column name')) throw err; }
    },
    ...
    () => db.exec(`
      CREATE TABLE IF NOT EXISTS traveloop_photo_cache_meta (
        cache_key  TEXT    PRIMARY KEY,
        content_type TEXT  NOT NULL DEFAULT 'image/jpeg',
        fetched_at INTEGER NOT NULL
      );
      CREATE INDEX IF NOT EXISTS idx_traveloop_photo_cache_meta_fetched_at ON traveloop_photo_cache_meta (fetched_at);
    `),
