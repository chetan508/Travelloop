...
const UA = 'TRAVELOOP Travel Planner (https://github.com/mauriceboe/TRAVELOOP)';
...
export async function resolveGoogleMapsUrl(url: string): Promise<{ lat: number; lng: number; name: string | null; address: string | null }> {
  ...
  const nominatimRes = await fetch(
    ...
    { headers: { 'User-Agent': 'TRAVELOOP-Travel-Planner/1.0' }, signal: AbortSignal.timeout(8000) }
  );
