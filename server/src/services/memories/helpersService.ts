...
export function canAccessUserPhoto(requestingUserId: number, ownerUserId: number, tripId: string, assetId: string, provider: string): boolean {
    ...
    if (tripId === '0') {
        const journeyPhoto = db.prepare(`
            ...
            JOIN traveloop_photos tkp ON tkp.id = gp.photo_id
            WHERE tkp.asset_id = ?
              AND tkp.provider = ?
              AND tkp.owner_id = ?
            LIMIT 1
    ...
    // Regular trip photos — join through traveloop_photos
...
// ── Unified photo access check (traveloop_photos based) ──────────────────────

export function canAccessTraveloopPhoto(requestingUserId: number, traveloopPhotoId: number): boolean {
    const photo = db.prepare('SELECT * FROM traveloop_photos WHERE id = ?').get(traveloopPhotoId) as { id: number; provider: string; owner_id: number | null } | undefined;
    ...
    const tripAccess = db.prepare(`
        ...
        LIMIT 1
    `).get(traveloopPhotoId, requestingUserId, requestingUserId);
