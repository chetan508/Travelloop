...
import { traveloopOAuthProvider, traveloopClientsStore } from './mcp/oauthProvider';
...
export function createApp(): express.Application {
  ...
  // the TRAVELOOP home page instead of the consent form.
  ...
  app.use('/oauth/authorize', mcpAddonGate, authorizationHandler({ provider: traveloopOAuthProvider }));

  // SDK DCR handler: accepts registrations without scope (fixes issue #959 bug 2)
  app.use('/oauth/register', mcpAddonGate, clientRegistrationHandler({ clientsStore: traveloopClientsStore }));

  // Token and revoke keep TRAVELOOP's own handlers (timing-safe hash comparison not supported by SDK clientAuth)
