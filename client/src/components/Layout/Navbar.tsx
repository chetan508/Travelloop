...
import { LogOut, Settings, ChevronDown, Shield, ArrowLeft, Users, CalendarDays, Briefcase, Globe, Compass } from 'lucide-react'
...
export default function Navbar({ tripTitle, tripId, onBack, showBack, onShare }: NavbarProps): React.ReactElement {
  ...
  return (
    <nav className="hidden md:flex items-center px-4 gap-4 fixed top-0 left-0 right-0 z-[200] traveloop-glass-nav" style={{
      background: dark
        ? (scrolled ? 'rgba(8,17,27,0.74)' : 'rgba(8,17,27,0.90)')
        : (scrolled ? 'rgba(255,255,255,0.72)' : 'rgba(255,255,255,0.88)'),
      ...
      <div className="flex items-center gap-3 min-w-0">
        {showBack && (
          <button onClick={onBack}
            className="traveloop-back-btn p-1.5 rounded-lg transition-colors flex items-center gap-1.5 text-sm flex-shrink-0"
            style={{ color: 'var(--text-muted)' }}
            onMouseEnter={e => e.currentTarget.style.background = 'var(--bg-hover)'}
            onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
            <ArrowLeft className="traveloop-back-icon w-4 h-4" />
            <span className="hidden sm:inline">{t('common.back')}</span>
          </button>
        )}

        <Link to="/dashboard" className="flex items-center transition-colors flex-shrink-0 group">
          <div style={{
            fontSize: '17px',
            fontWeight: '850',
        ...
        {globalAddons.length > 0 && !tripTitle && (
          <>
            ...
            <Link to="/explore"
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold transition-colors flex-shrink-0"
              style={{
                color: location.pathname === '/explore' ? 'var(--text-primary)' : 'var(--text-muted)',
                background: location.pathname === '/explore' ? 'var(--accent-soft)' : 'transparent',
              }}
              onMouseEnter={e => e.currentTarget.style.background = 'var(--bg-hover)'}
              onMouseLeave={e => { if (location.pathname !== '/explore') e.currentTarget.style.background = 'transparent' }}>
      ...
      {onShare && (
        <button onClick={onShare}
          className="flex items-center gap-1.5 py-1.5 px-3 rounded-full border transition-colors text-sm font-semibold flex-shrink-0"
      ...
      {user && (
        <div className="relative">
          <button onClick={() => setUserMenuOpen(!userMenuOpen)}
            className="flex items-center gap-2 py-1.5 px-2 rounded-full transition-colors border"
          ...
          {userMenuOpen && ReactDOM.createPortal(
            <>
              ...
              <div className="traveloop-menu-enter w-52 rounded-xl shadow-xl border overflow-hidden" style={{ position: 'fixed', top: 'var(--nav-h)', right: 8, zIndex: 9999, background: 'var(--bg-card)', borderColor: 'var(--border-primary)' }}>
                    ...
                    <div className="px-4 pt-2 pb-2.5 text-center" style={{ marginTop: 4, borderTop: '1px solid var(--border-secondary)' }}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: 'var(--bg-tertiary)', borderRadius: 99, padding: '4px 12px' }}>
                          <img src={dark ? '/text-light.svg' : '/text-dark.svg'} alt="TRAVELOOP" style={{ height: 10, opacity: 0.5 }} />
