...
Get TRAVELOOP running in under five minutes with a single Docker command.
...
**Why the encryption key?** TRAVELOOP encrypts stored secrets (API keys, MFA seeds, OIDC credentials) using this key. If you skip it, TRAVELOOP auto-generates one and saves it to `./data/.encryption_key`. Setting it explicitly means you control the key and can back it up separately.
...
On first boot TRAVELOOP automatically seeds an admin account before any user registers. The credentials depend on how you start the container:

- **With `ADMIN_EMAIL` and `ADMIN_PASSWORD` env vars set:** those values are used directly.
- **Without those env vars:** TRAVELOOP creates the account with email `admin@traveloop.local`, username `admin`, and a randomly generated password. The credentials are printed to the container log — run `docker logs traveloop` to retrieve them.
...
- [Reverse-Proxy](Reverse-Proxy) — put TRAVELOOP behind HTTPS (required for PWA install and secure cookies)
