...
function generateAccessToken(): string {
  return 'traveloopoa_' + randomBytes(32).toString('hex');
}

function generateRefreshToken(): string {
  return 'travelooprf_' + randomBytes(32).toString('hex');
}
...
export function createOAuthClient(
  ...
  const rawSecret   = isPublic ? null : 'traveloopcs_' + randomBytes(24).toString('hex');
...
export function rotateOAuthClientSecret(
  ...
  const rawSecret  = 'traveloopcs_' + randomBytes(24).toString('hex');
...
export function validateAuthorizeRequest(
  ...
  // RFC 8707 resource indicator: if provided, must identify the TRAVELOOP
  ...
  if (resource !== mcpResource) {
    return { valid: false, error: 'invalid_target', error_description: 'Requested resource must be the TRAVELOOP MCP endpoint' };
  }
