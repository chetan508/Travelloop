...
import { getOrCreateTraveloopPhoto, getOrCreateLocalTraveloopPhoto, setTraveloopPhotoProvider, deleteTraveloopPhotoIfOrphan } from './memories/photoResolverService';

function ts(): number {
  return Date.now();
}

// Per-entry photo view: join journey_entry_photos → journey_photos (gallery) → traveloop_photos.
...
const JP_JOIN = `journey_entry_photos jep
  ...
  JOIN traveloop_photos    tp ON tp.id  = gp.photo_id`;

// Per-journey gallery view: journey_photos → traveloop_photos (no entry context).
const GALLERY_SELECT = `
  gp.id, gp.journey_id, gp.photo_id, gp.caption, gp.shared, gp.sort_order, gp.created_at,
  tp.provider, tp.asset_id, tp.owner_id, tp.file_path, tp.thumbnail_path, tp.width, tp.height
`;
const GALLERY_JOIN = 'journey_photos gp JOIN traveloop_photos tp ON tp.id = gp.photo_id';
...
// Ensure a traveloop_photo_id is in the journey gallery; return its gallery row id.
function ensureInGallery(journeyId: number, traveloopPhotoId: number, caption?: string, shared?: number): number {
  const now = ts();
  const maxOrderRow = db.prepare('SELECT MAX(sort_order) as m FROM journey_photos WHERE journey_id = ?').get(journeyId) as { m: number | null };
  db.prepare(`
    INSERT OR IGNORE INTO journey_photos (journey_id, photo_id, caption, shared, sort_order, created_at)
    VALUES (?, ?, ?, ?, ?, ?)
  `).run(journeyId, traveloopPhotoId, caption || null, shared ?? 0, (maxOrderRow?.m ?? -1) + 1, now);
  const row = db.prepare('SELECT id FROM journey_photos WHERE journey_id = ? AND photo_id = ?').get(journeyId, traveloopPhotoId) as { id: number };
  return row.id;
}
...
export function addPhoto(entryId: number, userId: number, filePath: string, thumbnailPath?: string, caption?: string): JourneyPhoto | null {
  ...
  const traveloopPhotoId = getOrCreateLocalTraveloopPhoto(filePath, thumbnailPath);
  const galleryId = db.transaction(() => ensureInGallery(entry.journey_id, traveloopPhotoId, caption))();
...
export function addProviderPhoto(entryId: number, userId: number, provider: string, assetId: string, caption?: string, passphrase?: string): JourneyPhoto | null {
  ...
  const traveloopPhotoId = getOrCreateTraveloopPhoto(provider, assetId, userId, passphrase);
...
export function uploadGalleryPhotos(journeyId: number, userId: number, filePaths: { path: string; thumbnail?: string }[]): JourneyPhoto[] {
  ...
  for (const f of filePaths) {
    const traveloopPhotoId = getOrCreateLocalTraveloopPhoto(f.path, f.thumbnail);
    db.prepare(`
      INSERT OR IGNORE INTO journey_photos (journey_id, photo_id, shared, sort_order, created_at)
      VALUES (?, ?, 0, ?, ?)
    `).run(journeyId, traveloopPhotoId, nextOrder++, now);
    const row = db.prepare(`SELECT ${GALLERY_SELECT} FROM ${GALLERY_JOIN} WHERE gp.journey_id = ? AND gp.photo_id = ?`).get(journeyId, traveloopPhotoId);
...
export function addProviderPhotoToGallery(journeyId: number, userId: number, provider: string, assetId: string, caption?: string, passphrase?: string): any | null {
  if (!canEdit(journeyId, userId)) return null;
  const traveloopPhotoId = getOrCreateTraveloopPhoto(provider, assetId, userId, passphrase);
  const galleryId = db.transaction(() => ensureInGallery(journeyId, traveloopPhotoId, caption))();
...
export function deleteGalleryPhoto(journeyPhotoId: number, userId: number): { photo_id: number; file_path?: string | null } | null {
  ...
  const traveloopRow = db.prepare('SELECT file_path, provider FROM traveloop_photos WHERE id = ?').get(row.photo_id) as { file_path?: string; provider?: string } | undefined;
  ...
  deleteTraveloopPhotoIfOrphan(row.photo_id);

  return { photo_id: row.photo_id, file_path: traveloopRow?.file_path ?? null };
}

export function setPhotoProvider(photoId: number, provider: string, assetId: string, ownerId: number) {
  // photoId = journey_photos.id (gallery row); look up the traveloop_photo_id
  const jp = db.prepare('SELECT photo_id FROM journey_photos WHERE id = ?').get(photoId) as { photo_id: number } | undefined;
  if (!jp) return;
  setTraveloopPhotoProvider(jp.photo_id, provider, assetId, ownerId);
...
export function deletePhoto(photoId: number, userId: number): { id: number; photo_id: number; file_path?: string | null; journey_id: number } | null {
  ...
  return { id: row.id, photo_id: row.photo_id, file_path: traveloopRow?.file_path ?? null, journey_id: row.journey_id };
}
