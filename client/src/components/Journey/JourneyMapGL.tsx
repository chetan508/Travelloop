...
function ensureJourneyPopupStyle() {
  if (document.getElementById('traveloop-journey-popup-style')) return
  const s = document.createElement('style')
  s.id = 'traveloop-journey-popup-style'
  s.textContent = `
    .mapboxgl-popup.traveloop-journey-popup { pointer-events: none; animation: traveloop-journey-popup-in 180ms ease-out; }
    .mapboxgl-popup.traveloop-journey-popup .mapboxgl-popup-content {
    ...
    .mapboxgl-popup.traveloop-journey-popup.traveloop-dark .mapboxgl-popup-content {
      background: rgba(24, 24, 27, 0.88);
      border-color: rgba(255, 255, 255, 0.08);
      color: #FAFAFA;
    }
    .mapboxgl-popup.traveloop-journey-popup .mapboxgl-popup-tip {
      border-top-color: rgba(255, 255, 255, 0.94);
      border-bottom-color: rgba(255, 255, 255, 0.94);
    }
    .mapboxgl-popup.traveloop-journey-popup.traveloop-dark .mapboxgl-popup-tip {
      border-top-color: rgba(24, 24, 27, 0.88);
      border-bottom-color: rgba(24, 24, 27, 0.88);
    }
    .mapboxgl-popup.traveloop-journey-popup .mapboxgl-popup-close-button { display: none; }
    .traveloop-journey-popup-title {
      font-size: 13.5px;
      font-weight: 600;
    ...
    .mapboxgl-popup.traveloop-journey-popup.traveloop-dark .traveloop-journey-popup-title { color: #FAFAFA; }
    .traveloop-journey-popup-sub {
      display: flex;
    ...
    .mapboxgl-popup.traveloop-journey-popup.traveloop-dark .traveloop-journey-popup-sub { color: #A1A1AA; }
    .traveloop-journey-popup-place {
      min-width: 0;
      overflow: hidden;
      text-overflow: ellipsis;
    }
    .traveloop-journey-popup-sep {
      flex: 0 0 auto;
      opacity: 0.55;
      font-weight: 500;
    }
    .traveloop-journey-popup-date { flex: 0 0 auto; }
    @keyframes traveloop-journey-popup-in {
      from { opacity: 0; }
      to { opacity: 1; }
    }
  `
...
function markerHtml(dayColor: string, dayLabel: number, highlighted: boolean): HTMLDivElement {
  ...
  inner.className = 'traveloop-journey-marker-inner'
...
const JourneyMapGL = forwardRef<JourneyMapGLHandle, Props>(function JourneyMapGL(
  ...
  const showPopup = useCallback((id: string) => {
    ...
    if (place) subParts.push(`<span class="traveloop-journey-popup-place">${place}</span>`)
    if (date) subParts.push(`<span class="traveloop-journey-popup-date">${date}</span>`)
    const subline = subParts.length === 2
      ? `${subParts[0]}<span class="traveloop-journey-popup-sep">\u00B7</span>${subParts[1]}`
      : subParts.join('')

    const html = `
      <div class="traveloop-journey-popup-title">${primary}</div>
      ${subline ? `<div class="traveloop-journey-popup-sub">${subline}</div>` : ''}
    `
    ...
    if (popupRef.current) {
      ...
      if (el) el.classList.toggle('traveloop-dark', !!darkRef.current)
    } else {
      popupRef.current = new mapboxgl.Popup({
        ...
        anchor: 'bottom',
        offset,
        className: `traveloop-journey-popup${darkRef.current ? ' traveloop-dark' : ''}`,
        maxWidth: '280px',
      })
  ...
  const setMarkerStyle = useCallback((id: string, highlighted: boolean) => {
    ...
    const currentInner = el.querySelector('.traveloop-journey-marker-inner') as HTMLDivElement | null
    ...
    const nextInner = next.querySelector('.traveloop-journey-marker-inner') as HTMLDivElement
