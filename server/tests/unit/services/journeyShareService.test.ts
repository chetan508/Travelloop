...
/** Insert a traveloop_photos + journey_photos (gallery) + journey_entry_photos row and return the traveloop_photos id (used as photoId in public URLs). */
function insertJourneyPhoto(
  ...
  const traveloopResult = testDb.prepare(`
    INSERT INTO traveloop_photos (provider, asset_id, file_path, owner_id, created_at)
    VALUES (?, ?, ?, ?, ?)
  `).run(provider, opts.assetId ?? null, filePath, opts.ownerId ?? null, Date.now());
  const traveloopId = traveloopResult.lastInsertRowid as number;
  ...
  testDb.prepare(`
    ...
    VALUES (?, ?, NULL, 0, ?)
  `).run(journeyId, traveloopId, now);

  const galleryRow = testDb.prepare('SELECT id FROM journey_photos WHERE journey_id = ? AND photo_id = ?').get(journeyId, traveloopId) as { id: number };
  ...
  // Return traveloop_photos.id — this is p.photo_id in the public API response
...
describe('validateShareTokenForPhoto', () => {
  ...
  it('JOURNEY-SHARE-016: resolves correctly when traveloop_photos.id differs from journey_photos.id (Immich bulk-sync scenario)', () => {
    // Simulate a user who has many traveloop_photos from Immich syncs before adding a journey photo.
    // traveloop_photos.id will be higher than journey_photos.id — the previous bug matched on jp.id
    // instead of jp.photo_id, causing a 404 for Immich photos in public shares.
    const { user } = createUser(testDb);
    const journey = createJourney(testDb, user.id);
    const entry = createJourneyEntry(testDb, journey.id, user.id);

    // Pre-populate traveloop_photos to push the autoincrement higher
    for (let i = 0; i < 5; i++) {
      testDb.prepare(`INSERT INTO traveloop_photos (provider, asset_id, owner_id, created_at) VALUES ('immich', ?, ?, ?)`).run(`bulk-asset-${i}`, user.id, Date.now());
    }

    // This traveloop_photos row gets a high id (e.g. 6) while journey_photos id will be 1
    const traveloopPhotoId = insertJourneyPhoto(entry.id, { assetId: 'journey-asset-xyz', ownerId: user.id });
    const { token } = createOrUpdateJourneyShareLink(journey.id, user.id, {});

    // photoId = traveloop_photos.id (6), not journey_photos.id (1)
    const result = validateShareTokenForPhoto(token, traveloopPhotoId);
