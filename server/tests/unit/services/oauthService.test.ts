...
describe('createOAuthClient', () => {
  it('creates a client successfully and returns client_secret only on creation', () => {
    ...
    expect((result.client!.client_secret as string).startsWith('traveloopcs_')).toBe(true);
  });
...
describe('rotateOAuthClientSecret', () => {
  it('rotates secret and returns new client_secret starting with traveloopcs_', () => {
    const { user } = createUser(testDb);
    const created = makeClient(user.id);
    ...
    expect((result.client_secret as string).startsWith('traveloopcs_')).toBe(true);
...
describe('issueTokens + getUserByAccessToken', () => {
  it('issues tokens with correct prefixes', () => {
    ...
    expect(tokens.access_token.startsWith('traveloopoa_')).toBe(true);
    expect(tokens.refresh_token.startsWith('travelooprf_')).toBe(true);
  ...
  it('getUserByAccessToken returns null for unknown token', () => {
    expect(getUserByAccessToken('traveloopoa_unknown')).toBeNull();
  });
...
describe('refreshTokens', () => {
  it('exchanges a refresh token for a new token pair', () => {
    ...
    expect(result.tokens!.access_token.startsWith('traveloopoa_')).toBe(true);
  });
  ...
  it('returns invalid_grant for unknown refresh token', () => {
    ...
    const result = refreshTokens('travelooprf_unknown', clientId, rawSecret);
